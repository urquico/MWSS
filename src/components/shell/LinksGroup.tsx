...
function LinksGroup({
  ...
  return (
    <div className={`relative w-full ${variant === 'main' ? 'pl-2' : ''}`}>
      ...
      {variant === 'secondary' && (
        <div className='-pl-5 absolute left-0 top-0 z-0 h-full w-0.5 bg-slate-200' />
      )}
      <Box
      ...
      >
        ...
        {hasLinks && (
          <div className='flex items-center justify-center ml-auto'>
            <IconChevronRight
              className={`transform text-slate-500 duration-200 ease-in-out ${opened ? '-rotate-90' : 'rotate-0'}`}
            />
          </div>
        )}
      </Box>
      {hasLinks ? (
        <Collapse in={opened} className='pl-6'>
          {items}
        </Collapse>
      ) : null}
    </div>
